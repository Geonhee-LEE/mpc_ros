...
class FG_eval 
{
    public:
        ...
        void operator()(ADvector& fg, const ADvector& vars) 
        {
            ...
            for (int i = 0; i < _mpc_steps; i++) 
            {
              cout << "vars[] : " << vars[_cte_start + i] << vars[_etheta_start + i] << vars[_vx_start + i] << vars[_vy_start + i] << i << endl;
            ...
            cout << "-----------------------------------------------" <<endl;
            cout << "cost_cte, etheta, velocity: " << cost_cte << ", " << cost_etheta  << ", " << cost_vel_x << ", " << cost_vel_y << endl;
            

            // Minimize the use of actuators.
            for (int i = 0; i < _mpc_steps - 1; i++) {
  
              cout << "vars[] : " << vars[_angvel_start + i] << vars[_ax_start + i] << vars[_ay_start + i] << i << endl;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) 
{
    ...
    for (int i = _angvel_start; i < _ax_start; i++) 
    {
